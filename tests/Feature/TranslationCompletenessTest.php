...
{
    public function test_ojs_locale_catalog_is_available(): void
    {
        // https://github.com/pkp/ojs/tree/stable-3_5_0/locale
        // Excludes "und" (undetermined language).
        $ojsLocales = explode(' ', 'an ar az be bg bs bs_Latn ca ckb cnr cs da de dsb el en eo es es_MX et eu fa fa_AF fi fr fr_CA gd gl he hi hr hsb hu hy id is it ja ka kab kk ko ky lo lt lv mk mn mr ms nb_NO nl nn pl ps pt pt_BR ro ru rue se sid sk sl sq sr_Cyrl sr_Latn sv th tr uk ur uz uz_Latn vi zh_Hans zh_Hant');
        $configured = array_keys(config('app.locales'));
        sort($configured);

        $this->assertSame($ojsLocales, $configured);
